...
class FakeModule(object):
    ...
    def awscli_initialize(self, context):
        ...
        self.context.before_call(lambda **kwargs: self.events_seen.append(kwargs))
...
class TestPlugins(unittest.TestCase):
    ...
    def test_plugin_register(self):
        emitter = plugin.load_plugins(['__fake_plugin__'])
        self.assertTrue(self.fake_module.called)
        self.assertTrue(isinstance(emitter, plugin.HierarchicalEmitter))
    ...
    def test_event_hooks_can_be_passed_in(self):
        ...
        emitter = plugin.load_plugins(['__fake_plugin__'], event_hooks=hooks)
        emitter.emit('before_call')
