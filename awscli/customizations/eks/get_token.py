...
class GetTokenCommand(BasicCommand):
    ...
    DESCRIPTION = ("Get a token for authentication with an EKS cluster. "
    ...
    ARG_TABLE = [
        {
            'name': 'cluster-name',
            'help_text': ("Specify the name of the EKS cluster."),
            'required': True
        },
        {
            'name': 'role-arn',
            'help_text': ("Assume this role before signing this token."),
            'required': False
        }
    ]
