...
@pytest.mark.parametrize('fullname', ['botocore', 's3transfer'])
def test_find_spec_aliased_packages(fullname):
    underlying_finder = RecordingMetaPathFinder()
    alias_finder = TopLevelImportAliasFinder(underlying_finder)
    alias_finder.find_spec(fullname, None, target=None)
    assert underlying_finder.find_spec_calls == [
        (fullname, alias_finder._parent_path, None)
    ]
...
def test_find_spec_non_aliased_packages(fullname, provided_path, expected_path):
    underlying_finder = RecordingMetaPathFinder()
    alias_finder = TopLevelImportAliasFinder(underlying_finder)
...
def test_add_alias_finder_wraps_builtin_finder(fake_builtin_path_finder):
    ...
    _assert_classes(
        sys_meta,
        [TopLevelImportAliasFinder, type(fake_builtin_path_finder)],
    )
