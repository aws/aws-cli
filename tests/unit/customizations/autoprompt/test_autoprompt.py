...
class TestCLIAutoPrompt(unittest.TestCase):
    ...
    def test_add_arg_if_outfile_not_in_argtable(self):
        arg_table = {}
        autoprompt.add_auto_prompt(self.session, arg_table, self.driver)
...
def test_auto_prompt_config():
    # Each case is a 4-tuple with the following meaning:
    # First index is the command line --no-cli-auto-prompt override:
    #   True means --no-cli-auto-prompt is specified.
    #   False means --no-cli-auto-prompt is not specified.
    # Second index is the command line --cli-auto-prompt override:
    #   True means --cli-auto-prompt is specified.
    #   False means --cli-auto-prompt is not specified.
    # The third index is the result from get_config_variable
    #   This takes a value of either 'on' or 'off'.
    # The fourth index is a boolean indicating whether auto-prompt
...
def _assert_auto_prompt_runs_as_expected(case):
    no_cli_autoprompt_override, cli_autoprompt_override, \
        config, expected_result = case
    session = mock.Mock(spec=Session)
    session.get_config_variable.return_value = config
    prompter = mock.Mock(spec=autoprompt.AutoPrompter)
    argument = autoprompt.AutoPromptArgument(session, prompter)
    parsed_args = argparse.Namespace()
    parsed_args.no_cli_auto_prompt = no_cli_autoprompt_override
    parsed_args.cli_auto_prompt = cli_autoprompt_override
    argument.auto_prompt_arguments(
        args={},
        parsed_args=parsed_args
    )
    nose.tools.eq_(prompter.prompt_for_values.called, expected_result)
...
class TestAutoPrompter(unittest.TestCase):
    def setUp(self):
        ...
        self.auto_prompter.prompt_for_values = mock.Mock()

    def test_auto_prompter_returns_no_args_on_no_input(self):
        original_args = []
        self.auto_prompter.prompt_for_values.return_value = []
        prompted_values = self.auto_prompter.prompt_for_values(original_args)
        self.assertEqual(prompted_values, [])

    def test_auto_prompter_returns_split_args_on_command_input(self):
        original_args = ['ec2']
        self.auto_prompter.prompt_for_values.return_value = ['ec2', 'create-image']
        prompted_values = self.auto_prompter.prompt_for_values(original_args)
        self.assertEqual(prompted_values, ['ec2', 'create-image'])
