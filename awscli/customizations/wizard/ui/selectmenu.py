...
class FilterableSelectionMenuControl(SelectionMenuControl):
    ...
    def __init__(
        self, items, display_format=None, cursor='>', no_results_message=None
    ):
        ...
        self._no_results_message = (
            no_results_message or 'No matching items found'
        )
    ...
    def create_content(self, width, height):
        def get_line(i):
            if i == 0:
                filter_display = f'Search: {self._filter_text}_'
    ...
    def get_key_bindings(self):
        ...
        def app_result(event):
            if self._filtered_items:
                event.app.exit(result=self._filtered_items[self._selection])
