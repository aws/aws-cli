...
from awscli.botocore import client, exceptions, hooks, utils
from awscli.botocore.auth import AUTH_TYPE_MAPS, BaseSigner
...
class TestAutoGeneratedClient(unittest.TestCase):
    ...
    def test_client_with_nonstandard_signature_version(self):
        ...
        with mock.patch('awscli.botocore.client.AUTH_TYPE_MAPS', auth_types):
            service_client = creator.create_client(
                'myservice', 'us-west-2', credentials=self.credentials
            )
    ...
    def test_client_fips_region_transformation(self):
        creator = self.create_client_creator()
        with self.assertLogs('awscli.botocore.client', level='WARNING') as log:
    ...
    def test_signing_region_does_not_change_client_region(self):
        with mock.patch('awscli.botocore.args.RequestSigner') as mock_signer:
    ...
    @mock.patch('awscli.botocore.args.RequestSigner')
    ...
    def test_client_signature_override_arg(self, request_signer):
        creator = self.create_client_creator()
        config = awscli.botocore.config.Config(signature_version='foo')
        creator.create_client(
            'myservice',
            'us-west-2',
            credentials=self.credentials,
    ...
    def test_client_custom_user_agent_in_request(self):
        creator = self.create_client_creator()
        config = awscli.botocore.config.Config(user_agent='baz')
        service_client = creator.create_client(
            'myservice', 'us-west-2', client_config=config
        )
    ...
    def test_client_custom_user_agent_extra_in_request(self):
        creator = self.create_client_creator()
        config = awscli.botocore.config.Config(user_agent_extra='extrastuff')
        service_client = creator.create_client(
            'myservice', 'us-west-2', client_config=config
        )
        service_client.test_operation(Foo='one')
    ...
    @mock.patch('awscli.botocore.args.RequestSigner.sign')
    ...
    def test_client_with_custom_connect_timeout(self):
        self.create_client_creator().create_client(
            'myservice',
            'us-west-2',
            client_config=awscli.botocore.config.Config(connect_timeout=123),
        )
        call_kwargs = self.endpoint_creator.create_endpoint.call_args[1]
    ...
    def test_client_with_custom_read_timeout(self):
        self.create_client_creator().create_client(
            'myservice',
            'us-west-2',
            client_config=awscli.botocore.config.Config(read_timeout=234),
        )
        call_kwargs = self.endpoint_creator.create_endpoint.call_args[1]
    ...
    def test_can_register_standard_retry_mode(self):
        with mock.patch('awscli.botocore.client.standard') as standard:
            creator = self.create_client_creator()
            creator.create_client(
                'myservice',
                'us-west-2',
    ...
    def test_client_s3_addressing_style_with_incorrect_style(self):
        with self.assertRaises(InvalidS3AddressingStyleError):
            awscli.botocore.config.Config(s3={'addressing_style': 'foo'})
    ...
    def test_client_operation_hostname_binding_configuration(self):
        config = awscli.botocore.config.Config(inject_host_prefix=False)
    ...
    def test_client_operation_hostname_binding_configuration_none(self):
        config = awscli.botocore.config.Config(inject_host_prefix=None)
        client = self._create_hostname_binding_client(
            'us-west-2',
            client_config=config,
        )
...
class TestConfig(unittest.TestCase):
    def test_can_use_args_to_construct(self):
        config = awscli.botocore.config.Config(
            *awscli.botocore.config.Config.OPTION_DEFAULTS.values()
        )
        for (
            option,
            default_value,
        ) in awscli.botocore.config.Config.OPTION_DEFAULTS.items():
    ...
    def test_can_use_kwargs_to_construct(self):
        config = awscli.botocore.config.Config(
            **awscli.botocore.config.Config.OPTION_DEFAULTS
        )
        for (
            option,
            default_value,
    ...
    def test_can_use_mix_of_args_and_kwargs(self):
        config = awscli.botocore.config.Config('us-east-1', read_timeout=50)
    ...
    def test_invalid_kwargs(self):
        with self.assertRaisesRegex(TypeError, 'Got unexpected keyword'):
            awscli.botocore.config.Config(foo='foo')

    def test_pass_invalid_length_of_args(self):
        with self.assertRaisesRegex(TypeError, 'Takes at most'):
            awscli.botocore.config.Config(
                'foo', *awscli.botocore.config.Config.OPTION_DEFAULTS.values()
            )

    def test_create_with_multiple_kwargs(self):
        with self.assertRaisesRegex(TypeError, 'Got multiple values'):
            awscli.botocore.config.Config('us-east-1', region_name='us-east-1')

    def test_merge_returns_new_config_object(self):
        config = awscli.botocore.config.Config()
        other_config = awscli.botocore.config.Config()
        new_config = config.merge(other_config)
        # Check the type is correct
        self.assertIsInstance(new_config, awscli.botocore.config.Config)
    ...
    def test_merge_overrides_values(self):
        config = awscli.botocore.config.Config(region_name='us-east-1')
        other_config = awscli.botocore.config.Config(region_name='us-west-2')
        new_config = config.merge(other_config)
        self.assertEqual(new_config.region_name, 'us-west-2')

    def test_merge_overrides_values_even_when_using_default(self):
        config = awscli.botocore.config.Config(region_name='us-east-1')
        other_config = awscli.botocore.config.Config(region_name=None)
        new_config = config.merge(other_config)
        self.assertEqual(new_config.region_name, None)

    def test_merge_overrides_values_even_when_using_default_timeout(self):
        config = awscli.botocore.config.Config(read_timeout=30)
        other_config = awscli.botocore.config.Config(
    ...
    def test_can_set_retry_max_attempts(self):
        config = awscli.botocore.config.Config(retries={'max_attempts': 15})
    ...
    def test_validates_retry_config(self):
        with self.assertRaisesRegex(
            ...
            awscli.botocore.config.Config(retries={'not-allowed': True})

    def test_validates_max_retry_attempts(self):
        with self.assertRaises(InvalidMaxRetryAttemptsError):
            awscli.botocore.config.Config(retries={'max_attempts': -1})
        with self.assertRaises(InvalidMaxRetryAttemptsError):
            awscli.botocore.config.Config(retries={'max_attempts': 0})

    def test_validaties_retry_mode(self):
        with self.assertRaises(InvalidRetryModeError):
            awscli.botocore.config.Config(retries={'mode': 'turbo-mode'})
...
class TestClientEndpointBridge(unittest.TestCase):
    ...
    def test_use_dualstack_endpoint(self):
        config = awscli.botocore.config.Config(use_dualstack_endpoint=True)
    ...
    def test_use_fips_endpoint(self):
        config = awscli.botocore.config.Config(use_fips_endpoint=True)
        bridge = ClientEndpointBridge(self.resolver, client_config=config)
        bridge.resolve('ec2', 'us-west-2')
        self.resolver.construct_endpoint.assert_called_with(
            'ec2',
