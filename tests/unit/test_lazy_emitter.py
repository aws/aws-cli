...
class TestMainCommandTableOps:
    def test_covered_plugins_not_imported(self, mock_module):
        registry = {
            'building-command-table.main': [
                ('heavy.module', 'register_add_cmd', 'init'),
            ],
        }
        main_ops = [
            (
    ...
    def test_uncovered_plugins_still_initialized(self, mock_module):
        registry = {
            'building-command-table.main': [
                ('heavy.module', 'register_add_cmd', 'init'),
                ('other.module', 'do_something_else', 'init'),
            ],
        }
        ...
        mock_other.do_something_else = MagicMock()

        def import_side_effect(mod_path):
            if mod_path == 'other.module':
                return mock_other
            return mock_module
        ...
        # other.module should have been imported and initialized
        mock_other.do_something_else.assert_called_once()
        assert emitter.initialized_count == 2

    def test_rename_op_applied_to_command_table(self):
        registry = {
            'building-command-table.main': [
                ('rename.module', 'register_rename', 'init'),
            ],
        }
        main_ops = [
            (
    ...
    def test_main_ops_skips_covered_but_initializes_bare_prefix(
        ...
        registry = {
            'building-command-table': [
                ('global.module', 'register_global', 'init'),
            ],
            'building-command-table.main': [
                ('heavy.module', 'register_add_cmd', 'init'),
            ],
        ...
        # global.module should still be imported and initialized
        mock_global.register_global.assert_called_once()
        # Both entries should be marked initialized
        assert emitter.initialized_count == 2
