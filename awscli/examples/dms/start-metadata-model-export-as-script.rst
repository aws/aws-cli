...
The following ``start-metadata-model-export-as-script`` example queues an export of converted metadata models for all objects in the ``ExampleSchema`` schema as data definition language (DDL) scripts to the S3 bucket associated with the migration project. ::
