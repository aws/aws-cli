...
class TestStreamDisplay:
    ...
    def setup_method(self):
        self.display = StreamDisplay(use_color=True)

    def test_show_startup_message(self, capsys):
        """Test startup message includes timestamp"""
        self.display.show_startup_message()

        output = capsys.readouterr().out
        assert "Starting monitoring..." in output
        assert "[" in output

    def test_show_polling_message(self, capsys):
        """Test polling message includes timestamp"""
        self.display.show_polling_message()

        output = capsys.readouterr().out
        assert "Polling for updates..." in output

    def test_show_monitoring_data_with_info(self, capsys):
        """Test showing info message"""
        self.display.show_monitoring_data(None, "Info message")

        output = capsys.readouterr().out
        assert "Info message" in output and output.endswith("\n")

    def test_show_monitoring_data_first_poll_shows_all(self, capsys):
        ...
        self.display.show_monitoring_data(resource_group, None)

        output = capsys.readouterr().out
        assert "LoadBalancer" in output
        assert "lb-123" in output

    def test_show_monitoring_data_no_changes(self, capsys):
    ...
    def test_show_monitoring_data_with_new_resource(self, capsys):
        ...
        self.display.show_monitoring_data(resource_group1, None)
        capsys.readouterr()
        ...
        self.display.show_monitoring_data(resource_group2, None)

        output = capsys.readouterr().out
        assert "TargetGroup" in output

    def test_show_timeout_message(self, capsys):
        """Test timeout message"""
        self.display.show_timeout_message()

        output = capsys.readouterr().out
        assert "timeout reached" in output.lower()

    def test_show_service_inactive_message(self, capsys):
        """Test service inactive message"""
        self.display.show_service_inactive_message()

        output = capsys.readouterr().out
        assert "inactive" in output.lower()

    def test_show_completion_message(self, capsys):
        """Test completion message"""
        self.display.show_completion_message()

        output = capsys.readouterr().out
        assert "complete" in output.lower()

    def test_show_user_stop_message(self, capsys):
        """Test user stop message"""
        self.display.show_user_stop_message()
    ...
    def test_show_error_message(self, capsys):
        """Test error message"""
        self.display.show_error_message("Test error")
    ...
    def test_print_flattened_resources_with_reason(self, capsys):
        ...
        output = capsys.readouterr().out
        assert "LoadBalancer" in output
        assert "Reason: Waiting for DNS propagation" in output

    def test_print_flattened_resources_with_updated_at(self, capsys):
    ...
    def test_print_flattened_resources_with_additional_info(self, capsys):
        ...
        output = capsys.readouterr().out
        assert "LoadBalancer" in output
        assert "Info: DNS: example.elb.amazonaws.com" in output

    def test_print_flattened_resources_complete_multi_line(self, capsys):
        ...
        output = capsys.readouterr().out
        assert "LoadBalancer" in output and "lb-123" in output
        assert "Reason: Provisioning load balancer" in output
        assert "Last Updated At:" in output
        assert "Info: Type: application" in output

    def test_diff_detects_status_change(self, capsys):
    ...
    def test_diff_detects_reason_change(self, capsys):
    ...
    def test_diff_detects_additional_info_change(self, capsys):
    ...
    def test_resource_with_none_type_shows_identifier(self, capsys):
    ...
    def test_resource_with_none_type_and_none_identifier(self, capsys):
        """Test resources with both resource_type=None and identifier=None show only status"""
