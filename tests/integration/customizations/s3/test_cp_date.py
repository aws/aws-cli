...
class S3CpDateTest(awscli.testutils.BaseS3CLICommand):
    ...
    def extra_setup(self):
        ...
        # "dir" is removed
        # create a bunch of numbered files
        ver = "v5"
        self.files.create_file(os.path.join(ver, "foo"), "")
        self.files.create_file(os.path.join(ver, "bar"), "")
        self.sync_up(ver)
    ...
    def test_cp_date(self):
        """Using pre-recorded modification dates, recursively download each 
        version of the file tree with forced pagination and compare to the local
        file tree.

        Check that transfer of a single existing object not yet existed by the 
        date raises an error.

        Check that transfer of a single nonexisting object raises an error.
        """
        for ver in "v1", "v2", "v3", "v4", "v5":
            date = self.mod[ver].isoformat()
            res = awscli.testutils.aws(
                "s3 cp --recursive --page-size 1 --date %s %s %s" % (
                    date,
        ...
        res = awscli.testutils.aws("s3 cp --date 100 %s/foo %s" % (
            self.s3_scheme + self.t_bucket,
            self.files.full_path(self.s3_prefix + ver))
        )
        self.assertNotEqual(res, 0)
        res = awscli.testutils.aws("s3 cp --date 100 %s/nonexisting %s" % (
            self.s3_scheme + self.t_bucket,
            self.files.full_path(self.s3_prefix + ver))
        )
        self.assertNotEqual(res, 0)
