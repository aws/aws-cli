...
class Windows(System):
    ...
    def validate_administrator(self):
        if not ctypes.windll.shell32.IsUserAnAdmin():
            raise RuntimeError("You must run this command as an Administrator.")
    ...
    def install(self, params):
        ...
        not_found = "Cannot find any service with service name 'codedeployagent'"
        ...
        if "Running" not in output:
            raise RuntimeError("The AWS CodeDeploy Agent did not start after installation.")
    ...
    def _remove_agent(self):
        ...
        (output, error) = process.communicate()
        if process.returncode != 0:
            raise RuntimeError("Failed to uninstall the AWS CodeDeploy Agent:\n{0}".format(error))
...
class Linux(System):
    ...
    def install(self, params):
        ...
        subprocess.check_call(["./{0}".format(self.INSTALLER), "auto"], env=environment)
    ...
    def _stop_agent(self, params):
        process = subprocess.Popen(
            ["service", "codedeploy-agent", "stop"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        (output, error) = process.communicate()
