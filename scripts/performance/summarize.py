...
class Summarizer:
    ...
    def __init__(self, benchmark_prefix):
        self._benchmark_prefix = benchmark_prefix
        self.total_files = 0
        self._num_rows = 0
    ...
    def _detailed_json_benchmark(
        ...
        return {
            'name': f'{self._benchmark_prefix}.{metric_name}',
    ...
    def summarize_as_detailed_json(self):
        """Return detailed JSON summary of processed data, including additional metadata about
...
def main():
    ...
    parser.add_argument(
        ...
        '--output-format',
        default='table',
        choices=['table', 'json', 'detailed_json'],
        help=(
            'Specify what output format to use for displaying results. '
            'By default, a pretty printed table is used, but you can also '
            'specify "json" to display pretty printed JSON, or "detailed_json" '
            'to display more detailed pretty printed JSON.'
        ),
    )
    parser.add_argument(
        '--benchmark-name',
        default=None,
        help=(
            'Specify a name for this benchmark. This name will only be used in the output '
            'if "detailed_json" is specified as the output format.'
        ),
    )
    args = parser.parse_args()
    summarizer = Summarizer(args.benchmark_name)
    summarizer.process(args)
    if args.output_format == 'table':
        result = summarizer.summarize_as_table()
    elif args.output_format == 'json':
        result = json.dumps(summarizer.summarize_as_json(), indent=2)
    else:
