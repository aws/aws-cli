...
SAMPLE_RESPONSE = json.dumps(
    {
        'queryId': 'a64371c2-7808-4bab-9a44-9698171c9cda',
        'suggestions': [
            {
                'textExcerptSuggestion': {
                    ...
                    'suggestionBody': (
                        'The following naming rules apply for general '
                        'purpose buckets.'
                    ),
...
class TestBuildSearchPayload:
    ...
    def test_supported_locale_is_used_in_payload(self, locale):
        payload = build_search_payload('anything', locale)
        assert payload['locales'] == [locale]

    @pytest.mark.parametrize(
        'locale', ['en_gb', 'en_GB.UTF-8', 'nl_nl', 'C']
    )
    def test_unsupported_locale_falls_back_to_default(self, locale):
        payload = build_search_payload('anything', locale)
...
class TestResolveLocale:
    @pytest.mark.parametrize(
        ...
        [
            ('en_US.UTF-8', 'en_us'),
            ('en_GB.UTF-8', DEFAULT_LOCALE),
            ('fr_FR.UTF-8', 'fr_fr'),
            ('de-DE', 'de_de'),
            ('ja_JP.eucJP', 'ja_jp'),
            ('pt_PT.UTF-8', DEFAULT_LOCALE),
        ],
    )
    def test_resolves_multiple_os_locales(self, raw, expected):
        assert resolve_locale(raw) == expected

    @pytest.mark.parametrize('raw', ['', None, 'C', 'POSIX'])
    def test_unusable_locale_remains_empty(self, raw):
        assert resolve_locale(raw) == ''
...
class TestDetectLocale:
    ...
    def test_uses_supported_os_locale_when_env_unset(
        self, os_locale, expected
    ):
        # Simulates Windows, where LC_*/LANG variables are not set and the
        # locale comes from the OS as a BCP-47 tag.
        assert detect_locale({}, os_locale=lambda: os_locale) == expected

    def test_unsupported_authoritative_env_locale_uses_default(self):
        environ = {'LC_ALL': 'en_GB.UTF-8', 'LANG': 'de_DE.UTF-8'}
        assert detect_locale(environ, os_locale=lambda: '') == DEFAULT_LOCALE
...
class TestParseSearchResult:
    def test_flattens_and_sanitizes_suggestions(self):
        result = parse_search_result(SAMPLE_RESPONSE)
        assert result == [
            {
                'title': 'Bucket naming rules',
                'link': 'https://docs.aws.amazon.com/s3/naming.html',
    ...
    def test_skips_non_text_excerpt_suggestions(self):
        response = json.dumps(
            {
                'queryId': 'query-id',
                'suggestions': [
                    {'someOtherSuggestion': {'title': 'skip'}},
                    {'textExcerptSuggestion': {'title': 'keep'}},
                ],
            }
        )
        assert parse_search_result(response) == [{'title': 'keep'}]
...
class TestRenderText:
    def test_renders_title_and_link_without_suggestion_body(self):
        excerpts = parse_excerpts(SAMPLE_RESPONSE)
...
class TestRender:
    def test_json_format_returns_flattened_suggestions(self):
        rendered = json.loads(render(SAMPLE_RESPONSE, OutputFormat.JSON))
        assert rendered == parse_search_result(SAMPLE_RESPONSE)
        assert rendered[0]['summary'] == 'General purpose bucket naming rules.'
        assert {'suggestionBody', 'context', 'isCitable'}.isdisjoint(
            rendered[0]
        )
...
class TestDocsSearchCommand:
    def _command(self, raw_response=SAMPLE_RESPONSE, configured_output='json'):
        session = mock_session()
        session.get_config_variable.return_value = configured_output
        client = FakeDocSearchClient(raw_response)
        stream = io.StringIO()
        output_stream_context = mock.MagicMock()
        output_stream_context.__enter__.return_value = stream
        output_stream_factory = mock.Mock()
        output_stream_factory.get_output_stream.return_value = (
        ...
        return command, client, stream, session, output_stream_factory

    @staticmethod
    def _parsed_globals(output=None):
        return Namespace(output=output, query=None, color='off')

    def test_uses_configured_default_output(self):
        command, client, stream, session, output_stream_factory = (
            self._command(configured_output='json')
        )

        rc = command(
            ['S3 bucket naming rules'], self._parsed_globals(output=None)
        )

        assert rc == 0
        assert client.queries == ['S3 bucket naming rules']
        output = json.loads(stream.getvalue())
        assert output == parse_search_result(SAMPLE_RESPONSE)
        assert isinstance(output, list)
        assert output[0]['summary'] == 'General purpose bucket naming rules.'
        assert {'suggestionBody', 'context', 'isCitable'}.isdisjoint(output[0])
        session.get_config_variable.assert_called_once_with('output')
        output_stream_factory.get_output_stream.assert_called_once_with()

    def test_explicit_global_output_takes_precedence_over_config(self):
        command, _, stream, session, _ = self._command(
            configured_output='text'
        )

        rc = command(['query'], self._parsed_globals(output='json'))

        assert rc == 0
        assert json.loads(stream.getvalue()) == parse_search_result(
            SAMPLE_RESPONSE
        )
        session.get_config_variable.assert_not_called()

    def test_passes_structured_result_to_formatter(self):
        command, _, stream, _, _ = self._command()
        parsed_globals = self._parsed_globals(output='json')
        formatter = mock.Mock()

        with mock.patch(
            'awscli.customizations.docs.get_formatter',
            return_value=formatter,
        ) as get_formatter:
            rc = command(['query'], parsed_globals)

        assert rc == 0
        get_formatter.assert_called_once_with('json', parsed_globals)
        formatter.assert_called_once_with(
            DocsSearchCommand.NAME,
            parse_search_result(SAMPLE_RESPONSE),
            stream=stream,
        )
