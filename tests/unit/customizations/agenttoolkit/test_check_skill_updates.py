...
def _run_check(monkeypatch, agent_configs, args=None, latest_versions=None):
    """Run check-skill-updates and return (output, mocks)."""
    ...
    with (
        patch(
            'awscli.customizations.agenttoolkit.check_skill_updates.'
            'resolve_latest_version',
            resolve_latest,
        ),
        patch(
            'awscli.customizations.agenttoolkit.check_skill_updates.create_client',
            return_value=mock_client,
        ) as create,
    ):
        cmd = CheckSkillUpdatesCommand(
...
def test_check_skill_updates_no_agents(monkeypatch):
...
def test_check_skill_updates_no_skills_installed(tmp_path, monkeypatch):
...
def test_check_skill_updates_reports_available_update(tmp_path, monkeypatch):
...
def test_check_skill_updates_hides_up_to_date_by_default(
    tmp_path, monkeypatch
):
    # Only skills with an update pending are interesting by default, otherwise
    # the listing is dominated by rows the user cannot act on.
    make_skill(tmp_path, '.test-agent', 'aws-s3')
    result, _, _ = _run_check(
        monkeypatch,
        [make_config(tmp_path)],
...
def test_check_skill_updates_all_reports_up_to_date(tmp_path, monkeypatch):
    make_skill(tmp_path, '.test-agent', 'aws-s3')
    result, _, _ = _run_check(
        monkeypatch,
        [make_config(tmp_path)],
...
def test_check_skill_updates_all_mixes_current_and_outdated(
    tmp_path, monkeypatch
):
    make_skill(tmp_path, '.test-agent', 'aws-s3')
    make_skill(tmp_path, '.test-agent', 'aws-lambda')
    configs = [make_config(tmp_path)]
    versions = {'aws-s3': 'v2', 'aws-lambda': 'v1'}

    default_result, _, _ = _run_check(
        monkeypatch, configs, latest_versions=versions
    )
    assert [s['name'] for s in default_result['skills']] == ['aws-s3']

    all_result, _, _ = _run_check(
        monkeypatch, configs, args=['--all'], latest_versions=versions
    )
    assert [s['name'] for s in all_result['skills']] == [
        'aws-lambda',
        'aws-s3',
    ]
...
def test_check_skill_updates_unreadable_metadata_reports_null_version(
...
def test_check_skill_updates_looks_up_each_skill_once(tmp_path, monkeypatch):
...
def test_check_skill_updates_filtering_does_not_add_lookups(
    tmp_path, monkeypatch
):
    # Filtering happens after the version lookups, so hiding up-to-date rows
    # must not change how many calls we make.
    make_skill(tmp_path, '.agent-a', 'aws-s3')
    make_skill(tmp_path, '.agent-a', 'aws-lambda')
    configs = [
        make_config(tmp_path, detection_path=str(tmp_path / '.agent-a'))
    ]
    result, resolve_latest, _ = _run_check(
        monkeypatch,
        configs,
...
def test_check_skill_updates_with_agent_filter(tmp_path, monkeypatch):
    make_skill(tmp_path, '.agent-a', 'aws-s3')
    make_skill(tmp_path, '.agent-b', 'aws-s3')
    configs = [
        make_config(
    ...
    result, _, _ = _run_check(
        monkeypatch,
        configs,
        args=['--agent', 'agent-a', '--all'],
        latest_versions={'aws-s3': 'v1'},
    )
    assert [s['agent'] for s in result['skills']] == ['Agent A']


def test_check_skill_updates_shared_skills_dir_reported_once(
    tmp_path, monkeypatch
):
...
def test_check_skill_updates_invalid_agent(tmp_path, monkeypatch):
