...
@pytest.mark.asyncio
async def test_multipart_threshold_single_put_below(aws_cli, aws_config, tmp_path):
...
@pytest.mark.asyncio
async def test_multipart_threshold_multipart_above(aws_cli, aws_config, tmp_path):
    ...
    assert_create_multipart_upload(server.requests[0], Bucket="bucket", Key="big.bin")
    assert_upload_part(server.requests[1], Bucket="bucket", Key="big.bin")
    assert_complete_multipart_upload(server.requests[2], Bucket="bucket", Key="big.bin", UploadId="upload-id")


@pytest.mark.asyncio
async def test_multipart_chunksize_controls_part_count(aws_cli, aws_config, tmp_path):
    ...
    assert rc == 0, stderr.decode()
    assert_create_multipart_upload(server.requests[0], Bucket="bucket", Key="big.bin")
    part_reqs = [r for r in server.requests if r.method == "PUT" and "partNumber" in r.path]
    assert len(part_reqs) == 3, format_requests(server)
    assert_complete_multipart_upload(server.requests[4], Bucket="bucket", Key="big.bin", UploadId="upload-id")
...
async def test_use_accelerate_endpoint(aws_cli, aws_config, tmp_path):
    ...
    assert "s3-accelerate" in host, f"Expected s3-accelerate in host, got {host}"
...
async def test_payload_signing_disabled(aws_cli, aws_config, tmp_path):
    ...
    assert sha256 == "UNSIGNED-PAYLOAD", f"Expected UNSIGNED-PAYLOAD, got {sha256}"
...
async def test_use_dualstack_endpoint_false(aws_cli, aws_config, tmp_path):
    ...
    assert rc == 0, stderr.decode()
    host = server.requests[0].headers.get("host", "")
    assert "dualstack" not in host, f"Expected no dualstack in host, got {host}"
