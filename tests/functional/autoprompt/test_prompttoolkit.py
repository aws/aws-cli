...
class BasicPromptToolkitTest(unittest.TestCase):
    ...
    def get_buffer_is_visible_assertion(self, buffer_name):
        return lambda app: self.assertIn(
            buffer_name,
            self.get_all_visible_and_focusable_buffers(app)
        )
    ...
    def get_all_visible_and_focusable_buffers(self, app):
        return [
...
class TestDebugPanel(BasicPromptToolkitTest):
    ...
    def test_debug_panel_is_focusable_on_command_backslash(self):
        ...
        stubber.add_keypress(
            ...
            app_assertion=self.get_current_buffer_assertion('debug_buffer')
        )
        stubber.run(self.prompter.pre_run)
    ...
    def test_open_save_dialog_on_control_s(self):
        ...
        stubber.add_keypress(
            Keys.ControlS,
            app_assertion=self.get_current_buffer_content_assertion(
                expected_content='prompt_debug.log'
            )
        )
        stubber.run(self.prompter.pre_run)
